...
export default function QuickSaleModal({
  ...
  if (isConfirmed) {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-950/70 p-4 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-2xl animate-in zoom-in-95 duration-300">
          <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-green-100 shadow-inner">
            <CheckCircle2 className="h-10 w-10 text-green-600 animate-bounce" />
          </div>

          <h3 className="text-2xl font-extrabold text-gray-950">Sale Complete!</h3>
          ...
          <div className="mt-6 rounded-xl bg-gray-50 p-4 text-left text-xs text-gray-600 border border-gray-100">
            <div className="flex justify-between py-1 border-b border-gray-200/60">
              <span className="text-gray-500 font-medium">Card</span>
              <span className="font-semibold text-gray-900 truncate max-w-[200px]">{card.player_name}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-gray-200/60">
              <span className="text-gray-500 font-medium">Sold Price</span>
              <span className="font-mono font-bold text-gray-900">{formatCurrency(numSellPrice)}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-gray-200/60">
          ...
          <button
            type="button"
            onClick={onSuccess}
            className="mt-8 w-full rounded-xl bg-gray-950 px-6 py-3.5 text-sm font-semibold text-white shadow-lg hover:bg-gray-800 transition-all transform hover:scale-[1.01] active:scale-[0.99]"
          >
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-950/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-gray-200 bg-white p-6 shadow-2xl sm:p-8 animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-100 pb-5">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-red-50 text-red-600 font-bold">
              <DollarSign className="h-6 w-6" />
            </div>
            <div>
              <h2 className="text-xl font-extrabold text-gray-950">Quick Sell Card</h2>
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 bg-gray-50 text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-colors"
        ...
        {errorMsg && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-3.5 text-sm font-medium text-red-700 animate-shake">
        ...
        {/* Card Summary Banner */}
        <div className="mt-6 flex items-center gap-4 rounded-2xl border border-gray-200/80 bg-gradient-to-br from-gray-50 to-white p-4 shadow-sm">
          <div className="relative h-20 w-14 shrink-0 overflow-hidden rounded-xl border border-gray-200 bg-gray-100 flex items-center justify-center">
            {card.image_url ? (
              <Image
                src={card.image_url}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="rounded-md bg-gray-900 px-2 py-0.5 text-[10px] font-bold text-white uppercase tracking-wider">
                {formatGrade(card.grade_key)}
              </span>
              <span className="text-xs font-semibold text-gray-500">{card.sport}</span>
            </div>
            <h3 className="mt-1 text-base font-bold text-gray-950 truncate">
              {card.player_name}
            </h3>
          ...
          <div className="text-right shrink-0">
            <div className="text-[11px] font-medium text-gray-400">Cost Basis</div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full rounded-xl border border-gray-300 bg-gray-50/50 pl-8 pr-4 py-3 font-mono text-xl font-bold text-gray-950 focus:border-blue-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-600/10 transition-all"
                />
              </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 rounded-xl border p-3 text-left transition-all ${
                      isSelected
                        ? 'border-blue-600 bg-blue-50/80 text-blue-900 font-bold shadow-sm ring-2 ring-blue-600/20'
                        : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300 font-medium'
                    }`}
                  >
              ...
              {PAYMENT_METHODS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 rounded-xl border p-3 text-left transition-all ${
                      isSelected
                        ? `${item.color} font-bold shadow-sm ring-2 ring-current/20`
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-300 bg-white pl-9 pr-3 py-2.5 text-sm text-gray-900 focus:border-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-600/10"
                />
              </div>
            </div>
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 focus:border-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-600/10"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
          <button
            ...
            className="rounded-xl border border-gray-300 bg-white px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg hover:from-red-500 hover:to-rose-500 disabled:cursor-not-allowed disabled:opacity-50 transition-all transform hover:scale-[1.02] active:scale-[0.98]"
          >
            {isSubmitting ? (
              <>
