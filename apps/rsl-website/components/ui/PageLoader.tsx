...
export default function PageLoader() {
  ...
  useEffect(() => {
    // Show brief, ultra-smooth branded intro on first arrival
  ...
  // 2. Listen to route changes for the top progress bar
  useEffect(() => {
    // Trigger quick progress animation on pathname update
  ...
  useEffect(() => {
    const handleLinkClick = (e: MouseEvent) => {
      ...
      // Only trigger for internal page changes (not anchor jumps or external urls)
      if (
        href.startsWith('/') && 
        !href.startsWith('/#') && 
  ...
  return (
    <>
      {/* Top Navigation Progress Bar (Vercel / GitHub style) */}
      <div 
      ...
      {initialLoading && (
        <div 
          className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#050505] transition-all duration-500 ease-out select-none ${
            fadingOut 
