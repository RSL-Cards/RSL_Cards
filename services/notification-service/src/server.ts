...
import { closeQueue, startNotificationJobs } from "./config/queue.js";
...
async function main() {
  ...
  startNotificationJobs(env, app.log);

  await app.listen({ port, host: "0.0.0.0" });

  const fcmLine =
    env.FIREBASE_SERVICE_ACCOUNT && env.FIREBASE_SERVICE_ACCOUNT.length > 0
      ? "n FCM : Configured n"
      : "n FCM : Mock mode (no FIREBASE_SERVICE_ACCOUNT) n";
  const emailLine =
    env.RESEND_API_KEY && env.RESEND_API_KEY.length > 0
      ? "n Email : Configured n"
      : "n Email : Mock mode (no RESEND_API_KEY) n";
  printBanner({
  ...
  const shutdown = async (signal: string) => {
    ...
    await closeRedis();
    await closeQueue();
    app.log.info("notification-service shut down gracefully");
    process.exit(0);
  };
