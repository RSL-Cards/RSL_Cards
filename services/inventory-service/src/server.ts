...
import { closeQueue, startInventoryWorkers } from "./config/queue.js";
...
async function main() {
  ...
  startInventoryWorkers(env, app.log);
  ...
  printBanner({
    ...
    extras: ["n BullMQ Worker : inventory-aging - Running n"],
  });
  ...
  const shutdown = async (signal: string) => {
    ...
    await closeRedis();
    await closeQueue();
    app.log.info("inventory-service shut down gracefully");
    process.exit(0);
  };
