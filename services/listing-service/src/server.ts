...
import { closeQueue, startListingWorkers } from "./config/queue.js";
...
async function main() {
  ...
  const port = env.LISTING_SERVICE_PORT;
  ...
  startListingWorkers(env, app.log);
  ...
  const shutdown = async (signal: string) => {
    ...
    await closeRedis();
    await closeQueue();
    app.log.info("listing-service shut down gracefully");
    process.exit(0);
  };
